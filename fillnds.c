...
void fill_nondominated_sort (population *mixed_pop, population *new_pop, problem_instance *pi)
{
    ...
    do
    {
        ...
        if ( (archieve_size+front_size) <= popsize)
        {
            do
            {
                copy_ind (&mixed_pop->ind[temp2->index], &new_pop->ind[i], pi);
        ...
        {
            crowding_fill (mixed_pop, new_pop, i, front_size, elite, pi);
...
void crowding_fill (population *mixed_pop, population *new_pop, int count, int front_size, list *elite, problem_instance *pi)
{
    int *dist;
    list *temp;
    int i, j;
    ...
    {
        copy_ind(&mixed_pop->ind[dist[j]], &new_pop->ind[i], pi);
    }
    free (dist);
    return;
}
